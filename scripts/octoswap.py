...
LANG = {
    'vi': {
        ...
        # Tambahkan di bagian dictionary LANG untuk 'vi' dan 'en'
        'auto_selected': 'Đã chọn tự động',  # untuk 'vi'
        'no_balance': 'Không đủ số dư cho bất kỳ token nào',  # untuk 'vi'
    },
    'en': {
        'title': 'OCTO SWAP - MONAD TESTNET',
        'info': 'Info',
        ...
        'auto_selected': 'Auto-selected',     # untuk 'en'
        'no_balance': 'No sufficient balance for any token',  # untuk 'en'
...
async def approve_token(w3: Web3, private_key: str, token: dict, amount_wei: int, language: str = 'en') -> bool:
    ...
    try:
        ...
        if current_allowance >= amount_wei:
            print(f"{Fore.GREEN}  ✔ {token['name']} already approved with {current_allowance/(10**token['decimals'])} allowance{Style.RESET_ALL}")
            return True
        ...
        try:
            ...
            tx_params['gas'] = 180000
            print(f"{Fore.YELLOW}    {LANG[language]['gas_estimation_failed']}. {LANG[language]['default_gas_used'].format(gas=180000)}{Style.RESET_ALL}")
...
async def swap_token(w3: Web3, private_key: str, wallet_index: int, from_token: str, to_token: str, amount: float, swap_times: int, language: str = 'en'):
    ...
    for i in range(swap_times):
        try:
            ...
            # Get updated nonce for each transaction to avoid nonce issues
            ...
            # Add try/except for getAmountsOut
            try:
            ...
            try:
                ...
                try:
                    ...
                    tx_params['gas'] = 190000  # Increased default gas
